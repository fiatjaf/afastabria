...
extension ImageTool on ImageProvider {
  Future<Uint8List?> getBytes(final BuildContext context,
      {final ImageByteFormat format = ImageByteFormat.rawRgba}) async {
    final imageStream = resolve(createLocalImageConfiguration(context));
    final Completer<Uint8List?> completer = Completer<Uint8List?>();
    final ImageStreamListener listener = ImageStreamListener(
      (final imageInfo, final synchronousCall) async {
